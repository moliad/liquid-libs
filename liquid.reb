REBOL [
	...
	copyright:  "Copyright © 2004-2013 Maxim Olivier-Adlhoch"
	web:        http://www.moliad.net/modules/liquid/
   ...
   limitations under the License.}
	...
	v1.2.1  2012-11-14 (MOA)
		-we can now control HOW link resolving is merged the plug's mud (pipe or container).  See resolve-links? documentation for more details.
		
	v2.0.0	2013-06-14 (MOA)
		-first steps to make liquid R3 compatible.
		-changed license to Apache v2
		-possible divergence wrt R2 version.
...
;	vin: vout: none	
	...
	reindex-plug: select-plug: func [
		...
		either old-plug: select plug-list new-plug/sid [
			plug-list/(new-plug/sid): new-plug
			old-plug ; new in v2
		][
			to-error "LIQUID/reindex-plug() called on an unallocated plug."
		]
	]
...
;	
	...
	;-    count()
	;---
	while*: get in system/contexts/user 'while
	count: func [;
		series [series!]
		value
		/while wend
...
;		]
	...
	;-    process()
	;
	; purpose:  creates a simple plug which expets to be linked, building the valve/process() function
	...
	; returns:  
	;
	; notes:    - uses 'FUNCL and formulate
	;
	; tests:    
	;--------------------------
	process: funcl [
		...
		/base refplug [object!]  "uses !plug by default"
	][
		vin "processor()"
	...
	!plug: !default-plug: make object! [
		...
		; used to provide, with the difference that the container data is
		; always at the BEGINING of the data block provided to process.
		;
		; plugs with this property usually
		;
